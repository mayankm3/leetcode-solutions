...
public class TopKFrequentElements1 {
    ...
    // All of my remaining code iterations are simplifications of the last for loop
    // Someone's comment: when k is small, and n is large, its wasteful of both time and space
    ...
    public static int[] topKFrequent(int[] nums, int k) {
        ...
        int[] res = new int[k];
        for (int i = numsLength, j=0; i>0 ; i--) {
            if (bucket[i]!=null){
                for (int p = 0; p < bucket[i].size(); p++) {
                    res[j] = bucket[i].get(p);
                    j++;
                    k--;
                }
            }
